type MinStack struct {
    data []int
    curr_min int
    top int
}


func Constructor() MinStack {
    return MinStack{ data: make([]int, 0), top: -1, curr_min: math.MaxInt32 }
}


func (this *MinStack) Push(value int)  {
   if this.Size() == 0 {
      this.curr_min = value
      this.data = append(this.data, value)
   } else if value < this.curr_min {
      this.data = append(this.data, 2 * value - this.curr_min)
      this.curr_min = value
   } else {
      this.data = append(this.data,value)
   }

   this.top++
}


func (this *MinStack) Pop() {
    if this.top > -1 {
        peek := this.data[this.top]

        if peek < this.curr_min {
            this.curr_min = 2 * this.curr_min - peek
        }

        this.data = this.data[:this.top]
        this.top--
    }

    if this.top < 0 {
        this.data = make([]int, 0)
        this.curr_min = math.MaxInt32
        this.top = -1
    }
}


func (this *MinStack) Top() int {
    if this.top < 0 {
        return -1
    }

    peek := this.data[this.top]

    if peek < this.curr_min {
       return this.curr_min
    }

    return peek
}


func (this *MinStack) GetMin() int {
    return this.curr_min
}

func (this *MinStack) Size() int{
    return len(this.data)
}

/**
 * Your MinStack object will be instantiated and called as such:
 * obj := Constructor();
 * obj.Push(value);
 * obj.Pop();
 * param_3 := obj.Top();
 * param_4 := obj.GetMin();
 */